...
var finderParams=new URLSearchParams(location.search);document.querySelectorAll('.kira-finder select').forEach(function(select){if(finderParams.has(select.name))select.value=finderParams.get(select.name)});
...
function getKiraPropertiesPage() {
  return `<main class="kira-page"><section class="kira-page__hero"><div class="kira-page__hero-inner"><p class="kira-page__eyebrow">Addis Ababa properties</p><h1>Compare homes with clarity.</h1><p>Browse a focused demonstration collection with consistent photography, clear pricing and practical property details.</p></div></section>${kiraPropertyFinder}${kiraListingsSection}</main>`;
}
